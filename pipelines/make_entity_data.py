#!/usr/bin/env python3
"""
Create entity ranking training/test data.
Converts entity descriptions and qrels into training format.
"""
import collections
import json
import sys
...
def read_entity_data_file(file: str) -> Dict[str, str]:
    """Read entity data file with descriptions."""
    docs: Dict[str, str] = {}
...
def read_qrels(file: str) -> Dict[str, Tuple[List[str], List[str]]]:
    """Read qrels file and return positive and negative entities per query."""
...
def read_folds(file: str, index: int, train: bool):
    """Read topics from fold file."""
    with open(file, 'r') as f:
        key = "training" if train else "testing"
        return json.loads(f.read()).get(str(index))[key]


def to_pointwise_data_string(query_id, query, entities, desc, label, data):
    """Convert to pointwise training format."""
...
def to_data(query_id, query, pos_entities, neg_entities, desc, data):
    """Convert query with positive and negative entities to training data."""
...
def create_data(desc, qrels, queries, topics):
    """Create training data from descriptions, qrels, and queries."""
    data: List[str] = []

    for query_id in tqdm(queries, total=len(queries)):
        if topics is not None and query_id not in topics:
            continue
...
def main():
    ...
    # Experimentation
    parser.add_argument("--folds", help='File containing fold instructions.', required=False, type=str)
    parser.add_argument("--fold-index", help='Fold index.', required=False, type=int)
    ...
    if args.folds is not None and args.fold_index is not None:
        print('Reading folds...')
        fold_topics = read_folds(args.folds, args.fold_index, args.train)
        print(f"Limiting to queries: {fold_topics}")
        print('[Done].')
    else:
        fold_topics = None
